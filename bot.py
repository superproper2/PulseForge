    ...
    if data == "main_menu":
        ...
        for sport, league_name, league_id in popular:
            ...
            if fixtures:
                ...
                text += "\n"
        sent = bot.send_message(chat_id, text or "Сегодня нет матчей", parse_mode='Markdown')
        delayed_delete(chat_id, sent.message_id, delay=180)
...
def text_search(message):
    ...
    if len(query.split()) <= 3 and query.lower() in simple_sports:
        ...
        sent = bot.reply_to(message, f"Отлично! Спорт — **{sport.capitalize()}** 🔥\n\nЧто ищем?")
